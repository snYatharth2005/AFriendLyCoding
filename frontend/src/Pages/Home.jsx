      ...
      <div className="flex items-center gap-6">
        ...
        <div className="relative flex-shrink-0 w-[128px] h-[128px]">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            <span className="text-2xl font-bold font-mono text-[#f0f2f8] leading-none">
              {solved}
            </span>
            <span className="text-[11px] text-[#50566a] font-mono mt-0.5">
              /{total}
            </span>
            ...
            <div key={label}>
              ...
              <div className="h-1.5 bg-white/[0.06] rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${Math.min((count / t) * 100, 100)}%`,
                    background: color,
                  }}
                />
              </div>
...
const RecentSolves = ({ problems, loading }) => (
  <div className="bg-[#11131a] border border-white/[0.08] rounded-2xl p-6 shadow-[0_4px_20px_rgba(0,0,0,0.35)] h-full flex flex-col">
    <div className="flex items-center justify-between mb-5">
      <p className="text-[10px] font-mono text-[#50566a] tracking-[0.15em] uppercase">
        RECENT SOLVES
      </p>
      <Link
        to="/profile/me"
        className="text-xs text-[#00d084] hover:text-[#00b874] transition-colors font-medium"
      >
        See all →
      </Link>
    </div>
    ...
    <div className="flex-1 space-y-1 overflow-hidden">
      {loading ? (
        ...
        problems.slice(0, 8).map((p, i) => (
          ...
          >
            <div
              className={`w-2 h-2 rounded-full flex-shrink-0 ${diffColor(p.difficulty)}`}
            />
            <p className="flex-1 text-sm text-[#c8ccd8] font-mono truncate group-hover:text-[#f0f2f8] transition-colors">
              {p.id ? `${p.id}. ` : ""}
              {p.title}
            </p>
            <span className="text-xs text-[#50566a] flex-shrink-0 font-mono">
              {timeAgo(i)}
            </span>
          </div>
        ))
  ...
  useEffect(() => {
    ...
    getSolvedQuestion(localStorage.getItem("username"))
      .then((data) => {
        ...
        const found = data?.[0]?.users?.find(u => u.username === username);
        if (found) setLoggedUser(found);
      })
        ...
        <div className="flex-1 min-w-0 py-8 px-7">
          ...
          <div className="grid grid-cols-2 xl:grid-cols-4 gap-4 mb-6">
            ...
            <StatCard
              icon="🔥"
              label="Daily Streak"
              value={loading ? "…" : loggedUser.streak}
              sub={loading ? "…" : loggedUser.streak < 3 ? " Too Low bro! Work Hard!!" : "Keep it up!"}
              valueColor="text-[#ffc01e]"
            />
            <StatCard
              icon="📋"
              label="This Week"
              value={loading ? "…" : loggedUser.solvedInAWeek}
              sub="problems solved"
